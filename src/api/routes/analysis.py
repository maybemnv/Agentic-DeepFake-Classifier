...
@router.post(
    ...
    summary="Quick deepfake check",
    description="Fast analysis using only 5 frames."
)
async def quick_check(
    file: UploadFile = File(..., description="Video file")
):
    """
    Quick deepfake check using minimal frames.
    ...
    try:
        ...
        analyzer = DeepfakeAnalyzer(max_frames=5)
        result = analyzer.analyze(temp_path, show_progress=False)
