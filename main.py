...
Examples:
    ...
    parser.add_argument(
        "--video", "-v", type=str, required=True, help="Path to the video file"
    )
    parser.add_argument(
        "--weights", "-w", type=str, default=None, help="Path to model weights"
    )
    parser.add_argument(
        "--sample-rate", "-s", type=float, default=1.0, help="Frame sampling rate (fps)"
    )
    parser.add_argument(
        "--max-frames", "-m", type=int, default=None, help="Maximum frames to analyze"
    )
    parser.add_argument(
        "--quick", "-q", action="store_true", help="Quick check mode (5 frames)"
    )
    parser.add_argument(
...
def main():
    ...
    try:
        from src import DeepfakeAnalyzer, DeepfakeClassifier
